# Delegated second-pass review — Owner-approved conditional operating target

Task `GENIE_KEESURI_DELEGATED_REVIEW_GATE_001`. The 2026-09-11 Owner authority correction supersedes both the first preparation's blanket automatic-remediation shutdown proposal and the obsolete Mac-independent cloud requirement. Product-policy choices below are approved. The Owner's always-on Mac mini is the normal Work execution host. Once every documented code, review, delivery-eligibility, suppression, duplicate, transition and operational-evidence condition is factually satisfied, merge, OFF-mode deployment and delegated activation may proceed without another repetitive approval. **Until then LIVE DELEGATED SEND REMAINS OFF.** Conditional authority never creates missing consent or entitlement, converts shadow evidence into production authority, or permits a weakened guard.
...
1. Complete local exact-candidate/authentication, recipient/suppression, publication duplicate, repair-boundary and manual-mode tests. Verify the actual always-on Mac mini Work path, including scheduled Gmail/private image/render access, durable evidence and bounded failure notification. The reviewer and watchdog must not share one failure domain.
...
5. Close repository integration through a tested feature branch and review PR. Apply the 2026-09-11 conditional authority only after sufficient real always-on Mac mini evidence and every delivery gate passes. Merge first, deploy with delegated mode OFF, re-prove health/manual fallback, then activate the reviewed version. Production database, secret, IAM or customer-right mutation outside this sequence remains unauthorized. Repository integration does not turn a shadow PASS into production authority.
...
Controlled integration sequence: keep mode OFF; install prepared dependencies/configuration in an isolated environment; connect and verify the protected Gmail/final-render/reviewer/signing/suppression/notification chain on the always-on Mac mini; complete the finite shadow cohort and adjudicate anomalies/false holds; drain every old manual/delegated writer; reconcile prior accepted/unknown publications and seed the once-only cutover; verify manual fallback; merge; deploy with mode OFF; re-prove health; then activate only if every condition still holds. No key, IAM, DB, customer-right or customer-delivery production change is made merely by this repository patch.
