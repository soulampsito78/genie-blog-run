...
- image_prompt_studio (상단 히어로):
  ...
  - 상단 비율 하드락: 전신 또는 강한 3/4 샷이면 8.5~9등신 에디토리얼 패션 비율로 명확히 읽히게 구도·렌즈·자세를 서술한다. 필수 읽힘: 긴 다리 라인, 하이웨이스트 시각 밸런스, 긴 하체 실루엣, 슬림하지만 건강한 애슬레틱 체형, 우아한 수직 자세, 깨끗한 어깨선, 테일러링/실루엣으로 정의된 허리, 목선의 우아함. 짧다리·웅크림·토르소 과중·박스형·콤팩트 비율은 금지.
  - 상단 여성 매력 하드락: 여성스럽지만 프리미엄·세련·현실·신뢰 가능한 분위기. 과한 성적 연출 없이, 자신감·스타일링·실루엣·제스처·표정·에디토리얼 구도에서 매력이 나오게 한다. 눈은 부드럽지만 또렷하고(alert), 따뜻하지만 정돈된 기류, 은은한 생기. 딱딱한 마네킹 표정 금지.
  ...
  - 상단 네거티브 하드락: no short-legged read, no squat body, no compact torso-heavy figure, no boxy silhouette, no stiff catalogue mannequin pose, no flat symmetrical standing pose, no childlike proportions, no cheap glamour, no lingerie, no excessive cleavage, no nightclub styling, no over-sexualized pose.
  - 상단 구도 가드: premium briefing anchor로서 3/4 또는 full-body 허용. 인물을 작게 줄이는 news-thumb식 과도한 크롭 금지.
...
- image_prompt_outdoor (하단 일상/야외):
  ...
  - 구도·신체 하드락: 전신, 3/4, 또는 라이프스타일 미디엄 와이드 중 선택하되 가능하면 full-body 또는 medium-wide 환경 프레임을 우선한다. 전신/강한 3/4에서는 8.5~9등신 에디토리얼 비율을 명확히 유지한다(긴 다리 라인, 하이웨이스트 시각 밸런스, 긴 하체 실루엣, 슬림하지만 건강한 애슬레틱 체형, 우아한 수직 자세, 깨끗한 어깨선, 테일러링/실루엣으로 정의된 허리, 목선의 우아함). 짧다리·웅크림·토르소 과중·박스형·콤팩트 밸런스 금지.
  - 하단 여성 매력 하드락: 일상 장면이어도 여성스럽고 프리미엄·세련·현실·신뢰 가능한 매력을 유지한다. 과한 성적 연출 없이, 자신감 있는 움직임·스타일링·실루엣·제스처·표정·환경 구도에서 매력이 나오게 한다. 눈은 부드럽지만 또렷하고, 표정은 따뜻하지만 긴장감 있는 생기(soft but alert eyes, composed warmth, subtle vitality). 마네킹처럼 뻣뻣한 표정 금지.
  ...
  - 하단 네거티브 하드락: no short-legged read, no squat body, no compact torso-heavy figure, no boxy silhouette, no stiff catalogue mannequin pose, no flat symmetrical standing pose, no childlike proportions, no cheap glamour, no lingerie, no excessive cleavage, no nightclub styling, no over-sexualized pose.
  - 하단 적용: another blazer mannequin 금지, 움직임 기반 포즈 우선, 캐주얼이어도 긴 실루엣 유지, practical but attractive city styling.
...
- 상·하 이미지는 동일 인물이지만 **의상·포즈·장면·카메라 거리·배경 비중**이 확연히 달라야 한다. 회색 블레이저+연한 이너 같은 마네킹/카탈로그 반복, 상·하 동일 실루엣·동일 정면 앵커 포즈 금지. 서버 `image_exec_suffixes`가 런별 선택 계약을 덧붙이므로, JSON 프롬프트에서도 스튜디오 히어로 vs 야외 일상 전환의 대비가 문장만으로 읽히게 쓴다.
