...
# distribution rules must not be pasted wholesale into a generation prompt:
# doing so turns ordinary Korean expressions into absolute bans.  Kee-Suri
    ...
    if program_id == PROGRAM_GLOBAL:
        sections.extend(
            [
                ...
                "",
                "GLOBAL NATURAL KOREAN EDITING (generation-safe; source fidelity first)",
                *GLOBAL_NATURAL_KOREAN_RULES,
                "",
...
def build_keysuri_generation_prompt_compact(
    ...
    *,
    ...
    if _is_global_program_id(program_id):
        insert_at = sections.index("ALLOWED SOURCE IDS")
